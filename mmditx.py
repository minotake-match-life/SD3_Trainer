...
from other_impls import Mlp, SwiGLUFeedForward, attention, modulate, split_qkv
...
# these functions are not used, ckpt has already had pos emb 
...
# embedding for ADM class labels
...
class SelfAttention(nn.Module):
    ...
    # don't use this forward function
    def forward(self, x: torch.Tensor) -> torch.Tensor:
...
class DismantledBlock(nn.Module):
    ...
    def __init__(
        ...
        pre_only: bool = False, # True only in the last layer for context
        rmsnorm: bool = False,
        scale_mod_only: bool = False,
        swiglu: bool = False,
        qk_norm: Optional[str] = None, # "rms"
        x_block_self_attn: bool = False, # True for the first 13 layers
        dtype=None,
        device=None,
        **block_kwargs,
    ):
        ...
        # optional second self-attention layer for first 13 x blocks
        ...
        # After attention layer; LNnorm and MLP
        if not pre_only:
            if not rmsnorm:
        ...
        # settings
        self.scale_mod_only = scale_mod_only # False
        self.pre_only = pre_only
...
def block_mixing(context, x, context_block, x_block, c):
    ...
    # image post attention
    if x_block.x_block_self_attn:
        # for the first 13 x blocks
...
class JointBlock(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        # separate to_qkv and joint_attention, separate to_post_attention
        return block_mixing(
            *args,
...
class MMDiTX(nn.Module):
    ...
    def __init__(
        ...
        patch_size: int = 2,
        in_channels: int = 4, # 16
        depth: int = 28, # 24
        mlp_ratio: float = 4.0,
        learn_sigma: bool = False,
        adm_in_channels: Optional[int] = None, # 2048
        context_embedder_config: Optional[Dict] = None, #4096 -> 1536
        register_length: int = 0,
        rmsnorm: bool = False,
        scale_mod_only: bool = False,
        swiglu: bool = False,
        out_channels: Optional[int] = None, # = in_channels
        pos_embed_scaling_factor: Optional[float] = None,
        pos_embed_offset: Optional[float] = None,
        pos_embed_max_size: Optional[int] = None, # 384
        num_patches=None, # 384**2
        qk_norm: Optional[str] = None, # "rms"
        x_block_self_attn_layers: Optional[List[int]] = [], # first 13 layers
        qkv_bias: bool = True,
        dtype=None,
        device=None,
        verbose=False,
        ...
        # set num heads to be equal to depth, so each head has 64 dims
        num_heads = depth
        self.num_heads = num_heads
        ...
        # T5 and CLIP hidden states are projected to hidden_size
        ...
        # just use a sincos positional embedding from ckpt
        ...
        #  dual attention blocks in the first 13 transformer layers
        ...
        # modulation + linear, to produce patch_size**2 * out_channels channels
    ...
    def forward_core_with_concat(
        ...
        # ignore
        if self.register_length > 0:
            print(f">>B>>> context shape before cat: {None if context is None else context.shape}, x shape: {x.shape}, register shape: {self.register.shape}")
            context = torch.cat(
                (
        ...
        # final layer norm, modulation, linear, expand patch_size**2
        x = self.final_layer(x, c_mod)  # (N, L, patch_size ** 2 * out_channels)
    ...
    def forward(
        ...
        hw = x.shape[-2:]
        x = self.x_embedder(x) + self.cropped_pos_embed(hw) # (N, L=H'W', D)

        # timestep and class label embedding
        c = self.t_embedder(t, dtype=x.dtype)  # (N, D)
        if y is not None:
            y = self.y_embedder(y)  # (N, D)
            c = c + y  # (N, D)

        # 4096 -> 1536 (hidden_size)
        context = self.context_embedder(context)

        # 24 joint blocks and final layer
        x = self.forward_core_with_concat(x, c, context, skip_layers, controlnet_hidden_states)

        # just reshape to image
