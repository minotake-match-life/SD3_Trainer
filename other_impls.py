...
class ClipTokenWeightEncoder:
    def old_encode_token_weights(self, token_weight_pairs):
        """
        token_weight_pairs: [ (token_id, weight), ... ] 
        """
        tokens = list(map(lambda a: a[0], token_weight_pairs[0])) # (token_id, weight) -> [token_id]
        out, pooled = self([tokens]) # [[token_id]], out: [1, 77, C], pooled: [1, C'] 
    ...
    # we add a batch version
    def encode_token_weights(self, token_weight_pairs):
        """
        - token_weight_pairs: [ [ (token_id, weight), ... ], ... ]
        Returns:
          - out:    Tensor [B, 77, C] on cpu
          - pooled: Tensor [B, C'] on cpu
        """

        batch_tokens = []
        for twp in token_weight_pairs:
            tokens = list(map(lambda a: a[0], twp))  # (token_id, weight) -> [token_id]
            batch_tokens.append(tokens)

        out, pooled = self(batch_tokens)  # out: [B, 77, C], pooled: [B, C'] or None
        if pooled is not None:
            pooled = pooled.cpu()

        return out.cpu(), pooled        
...
class SDClipModel(torch.nn.Module, ClipTokenWeightEncoder):
    ...
    def forward(self, tokens):
        ...
        tokens = torch.LongTensor(tokens).to(device) #int64
...
class T5XXLTokenizer(SDTokenizer):
    ...
    def __init__(self):
        super().__init__(
            ...
            pad_to_max_length=True, # modify
            max_length=77, # modify
            min_length=77,
        )
